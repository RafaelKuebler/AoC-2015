...
def solve(strings: List[str], part2: bool = False) -> Optional[int]:
    final_dist = None
    all_places: Set[str] = set()
    distances: Dict[FrozenSet[str], int] = {}
    ...
    for solution in permutations(all_places):
        ...
        if final_dist is None:
            ...
            final_dist = max(final_dist, total)
        else:
            final_dist = min(final_dist, total)
