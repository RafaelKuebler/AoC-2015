...
def parse(lines: List[str]) -> List[List[int]]:
    regex = re.compile(
        r"(?P<name>\w+): capacity (?P<capacity>-?\d+), durability (?P<durability>-?\d+), "
    ...
    for i in range(100):
        for j in range(100 - i):
            for k in range(100 - i - j):
                ...
                durability = (
                    ingredients[0][1] * i + ingredients[1][1] * j + ingredients[2][1] * k + ingredients[3][1] * l
                )
                ...
                if part2:
                    calories = (
                        ingredients[0][4] * i + ingredients[1][4] * j + ingredients[2][4] * k + ingredients[3][4] * l
                    )
                    if calories != 500:
                        continue
